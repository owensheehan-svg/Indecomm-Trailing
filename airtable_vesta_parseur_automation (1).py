...
class IndecommAutomation:
    ...
    def fetch_vesta_loan(self, loan_number: str) -> Optional[Dict]:
        """Fetch loan data from Vesta API by loan number (two-step process)"""
        headers = {
        ...
        try:
            # Step 1: Get loan GUID from loan number
            lookup_url = f'{VESTA_BASE_URL}/v1/loans/by-number/{loan_number}'
            print(f"Looking up Vesta GUID for loan {loan_number}")
            
            response = requests.get(lookup_url, headers=headers)
            
            if response.status_code == 404:
                self.log_warning(loan_number, 'Vesta API', 'Loan number not found in Vesta')
                return None
            
            response.raise_for_status()
            loan_guid = response.text.strip('"')  # Response is just a string GUID
            
            if not loan_guid:
                self.log_error(loan_number, 'Vesta API', 'No GUID returned from loan number lookup')
                return None
            
            print(f"Found Vesta GUID: {loan_guid}")
            
            # Step 2: Get full loan data using GUID
            loan_url = f'{VESTA_BASE_URL}/v1/loans/{loan_guid}'
            print(f"Fetching full loan data from Vesta")
            
            response = requests.get(loan_url, headers=headers)
            response.raise_for_status()
            ...
            if e.response.status_code == 404:
                ...
                self.log_error(loan_number, 'Vesta API', 'Authentication failed - check API key')
            else:
